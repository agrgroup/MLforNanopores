...
def optimize_lgbm_rmse(trial, X, y):
    '''
...
def tune_cb(X, y_c, iter=100):
    ...
    study = optuna.create_study(direction='maximize', study_name='best_params', storage='sqlite:///optim_cb.db', load_if_exists=True)
...
def tune_lgbm(X, y, t, iter=100):
    ...
    '''
    optimization_func = partial(optimize_lgbm_rmse, X=X[y>=t], y=y[y>=t])
