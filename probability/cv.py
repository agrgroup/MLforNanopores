...
# Extract the targets (nanopore probability) and features (nanopores descriptors)
X, y = train.drop(columns='probability'), train['probability']
X, y = X[y<0.4].reset_index(drop=True), y[y<0.4].reset_index(drop=True)
...
classes = [1 if y[idx] >= t else 0 for idx in range(len(y))]
